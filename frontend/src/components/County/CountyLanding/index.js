        ...
        <div className="mainCountyLanding">
            ...
            <section className="sampleSection">
                ...
                <div className="sampleGrid">
                    {/* Example: you can flesh this out with real data */}
                    {countyList.slice(0, 3).map(county => (
                        <div key={county.id} className="sampleCard">
                            <h3>{county.name}</h3>
                            <p>Random judge: {county.randomJudge?.name || "N/A"}</p>
                            <p>Random crime: {county.randomCrime?.crime || "N/A"}</p>
                            <Link to={`/counties/${county.id}`}>See More</Link>
                        </div>
                    ))}
                </div>
            </section>
